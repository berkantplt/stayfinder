...
@section('content')
@php($isRemoved = $agency === null) {{-- acenta admin tarafından arşivlendi --}}
@php($isRejected = ! $isRemoved && $agency->isRejected())
@php($cardAccent = ($isRejected || $isRemoved) ? '#dc2626' : '#0d9488')
@php($cardSoft = ($isRejected || $isRemoved) ? '#fef2f2' : '#f0fdfa')

<div class="container" style="max-width:720px;padding-top:80px;padding-bottom:100px;">
    <div style="background:{{ $cardSoft }};border:1px solid {{ ($isRejected || $isRemoved) ? '#fecaca' : '#99f6e4' }};border-radius:28px;padding:40px 36px;box-shadow:0 24px 48px rgba(15,23,42,0.08);">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:22px;">
            ...
            <div>
                <div style="font-size:13px;font-weight:700;letter-spacing:0.3px;text-transform:uppercase;color:{{ $cardAccent }};">{{ $isRemoved ? 'Acenta Hesabı' : 'Acenta Başvurusu' }}</div>
                <h1 style="font-size:30px;font-weight:800;color:#0f172a;letter-spacing:-0.5px;">
                    {{ $isRemoved ? 'Acenta Hesabınız Kaldırıldı' : ($isRejected ? 'Başvurunuz Sonuçlandı' : 'Başvurunuz İnceleniyor') }}
                </h1>
            </div>
        </div>
        ...
        <div style="font-size:15px;color:#334155;line-height:1.8;">
            @if($isRemoved)
                Bağlı olduğunuz acenta hesabı yönetici tarafından kaldırıldı; turlarınız yayında değil ve acenta paneli kapalı.
                Bunun bir hata olduğunu düşünüyorsanız bizimle iletişime geçin.
            @elseif($isRejected)
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin-top:24px;">
            <div style="background:white;border:1px solid rgba(148,163,184,0.18);border-radius:18px;padding:16px;">
                ...
                <div style="font-size:20px;font-weight:800;color:#0f172a;margin-top:6px;">
                    {{ $isRemoved ? 'Kaldırıldı' : ($isRejected ? 'Reddedildi' : 'Onay Bekliyor') }}
                </div>
            </div>
            <div style="background:white;border:1px solid rgba(148,163,184,0.18);border-radius:18px;padding:16px;">
