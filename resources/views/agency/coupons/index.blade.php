    ...
    <div>
        ...
        <div class="section" style="padding:0;">
            <div class="admin-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; max-width:94%; margin-left:auto; margin-right:auto;">
                <div>
                    <h1 style="font-size:24px; font-weight:700; color:var(--text);">Kuponlarım</h1>
                    <p style="color:var(--text-sec); font-size:14px; margin-top:4px;">Sadece kendi kupon kodlarını oluştur ve yönet.</p>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align:right;">
                                            <div style="display:flex; gap:8px; justify-content:flex-end;">
                                                <form action="{{ route('agency.coupons.toggle', $coupon) }}" method="POST">
                                                    @csrf
                                                    <button class="btn btn-outline btn-sm">
                                                        {{ $coupon->is_active ? 'Pasife Al' : 'Aktifleştir' }}
                                                    </button>
                                                </form>
                                                <form action="{{ route('agency.coupons.destroy', $coupon) }}" method="POST" onsubmit="return confirm('Emin misiniz?');">
                                                    @csrf @method('DELETE')
...
<div id="createModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); align-items:center; justify-content:center; padding:20px; z-index:1000;">
    <div style="background:white; border-radius:16px; padding:32px; width:100%; max-width:600px; max-height:90vh; overflow-y:auto;">
        ...
        <form action="{{ route('agency.coupons.store') }}" method="POST">
            @csrf
            ...
            <div style="display:flex; gap:16px; margin-bottom:16px;">
                <div style="flex:1;">
                    <label style="display:block; font-size:13px; font-weight:600; margin-bottom:6px;">Minimum Sepet Tutarı (TL)</label>
