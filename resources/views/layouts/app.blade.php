...
<body class="{{ request()->is('admin*') || request()->is('agency*') || request()->is('acenta', 'acenta/*') ? 'panel-layout-active' : '' }} {{ request()->routeIs('home') ? 'm-hero-head' : '' }} {{ \App\Support\ChatV2Visibility::visible(request()) ? '' : 'fab-yok' }}">
                ...
                <div class="ftr-inner">
                    ...
                    <div class="ftr-trust">
                        ...
                        <div>
                            ...
                            <span><b>Doğrulanmış Acentalar</b><span>Her acenta yayına admin onayıyla çıkar.</span></span>
                        </div>
                        <div>
    ...
    {{-- ===== AI tur kartı: TEK ortak üretici =====
         Sohbet v2 (#cv2) ve eşleştirme testi ortak kullanır; her sayfada
         KOŞULSUZ yüklenir, hiçbir görünürlük guard'ının içine alınmamalı. --}}
    <script>
    ...
    @if(config('ai.quiz_enabled'))
    {{-- ⚠️ Eşleştirme testinin BAĞIMSIZ giriş noktası — sohbet guard'larının DIŞINDA.
         turxturQuiz modülü sohbetten bağımsız (LLM'siz) çalışıyor ama tek
         açıcısı bir dönem sohbet widget'ının guard'ının içindeydi: sohbet
         kapandığında teste hiçbir yerden ulaşılamıyordu. Bu modal kendi kabını
         sağladığı için sohbet widget'ı olmadan da açılır.
         window.openTurxturQuiz() her sayfadan çağrılabilir. --}}
    ...
    {{-- ⚠️ Tur karşılaştırma: SOHBET GUARD'ININ DIŞINDA durmalı.
         ...
         bir dönem sohbet widget'ının guard'ının içindeydi; sohbet kapandığında
         (ve admin kullanıcılarda / acenta sayfalarında) fonksiyonlar tanımsız kalıyor,
