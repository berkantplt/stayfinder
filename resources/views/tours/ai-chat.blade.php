                ...
                @if($messages->isEmpty())
                    <div style="max-width:540px;margin:32px auto;text-align:center;">
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;">
                            <button type="button" class="btn btn-outline btn-sm" id="quiz-entry" style="border-color:#0d9488;color:#0d9488;">🧭 Tur Eşleştirme Testi</button>
...
(function() {
    ...
    function appendAssistant(content, tours, isClarification) {
        ...
        if (tours && tours.length) {
            renderTourCards(grid, tours, null);
        } else {
    ...
    // Tur kartlari TEK ortak ureticiden gelir (layouts/app.blade.php:
    // window.turxturAiCard) - widget ile ayni yapi, light tema.
    function renderTourCards(grid, tours, logId) {
        if (!tours || !tours.length) return;
        tours.forEach((t, i) => {
            grid.appendChild(window.turxturAiCard.build(t, { theme: 'light', index: i, logId: logId }));
        });
    }
    ...
    // Tur eşleştirme testi girişi (boş-konuşma ekranındaki çip) — sonuçlar
    // bileşen içinde LLM'siz render edilir, sohbet akışına bağlanmaz
    document.getElementById('quiz-entry')?.addEventListener('click', () => {
        window.turxturQuiz.open({ container: messages, theme: 'light' });
        messages.scrollTop = messages.scrollHeight;
    });
